...
// 2. CÁLCULO AUTOMÁTICO DE CATEGORÍAS (Para index_ciar.html)
...
// 3. REGISTRO Y ACTUALIZACIÓN DE ATLETAS (index_ciar.html)
...
// 4. LÓGICA DE TABLA PÚBLICA (index_listaciar.html)
            ...
            tAdm.insertAdjacentHTML('beforeend', `
                <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                    <td class="px-4 py-3 uppercase">
                        ...
                        <div class="text-[9px] text-blue-500 font-bold">C.I: ${r.cedula || 'S/N'}</div>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <span class="${estCol} px-2 py-0.5 rounded-full text-[8px] font-black uppercase">${r.estatus}</span>
                    </td>
                    <td class="px-4 py-3 text-center">
                        <div class="flex justify-center gap-1">
                            <button onclick='window.editar("${doc.id}", ${JSON.stringify(r).replace(/'/g, "&apos;")})' class="bg-blue-600 text-white px-2 py-1 rounded-lg text-[8px] font-black hover:bg-blue-700">MOD</button>
                            <button onclick="window.eliminar('${doc.id}')" class="bg-red-600 text-white px-2 py-1 rounded-lg text-[8px] font-black hover:bg-red-800">DEL</button>
                        </div>
                    </td>
                </tr>
            `);
...
window.eliminar = async (id) => {
    if(confirm("⚠️ ¿DESEA ELIMINAR ESTE ATLETA DEFINITIVAMENTE DE DATA CIAR?")) {
        try {
...
// 7. MOTOR DE BÚSQUEDA DUAL (NOMBRE / CÉDULA)
window.filtrar = () => {
    const input = d.getElementById('busNom');
    if(!input) return;
    const val = input.value.toUpperCase();
    const filas = d.querySelectorAll('tbody tr');
    
    filas.forEach(tr => {
        // Al usar innerText, el sistema busca en todo el texto visible de la fila
        const textoFila = tr.innerText.toUpperCase();
        tr.style.display = textoFila.includes(val) ? "" : "none";
    });
};

// 8. PROTECCIÓN DE CÓDIGO (SISTEMATIKOS)
d.addEventListener('contextmenu', e => e.preventDefault());
d.onkeydown = function(e) {
    // Bloqueo de F12, Ctrl+U, Ctrl+Shift+I
    if (e.keyCode == 123 || 
